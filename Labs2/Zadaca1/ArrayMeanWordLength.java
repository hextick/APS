...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        for (int k=0;k<arr.getSize();k++){
            averageLength+= arr.get(k).length();
        }
        averageLength= (float) (averageLength/arr.getSize());
        for (int j = 1; j < arr.getSize(); j++) {
            if(abs(averageLength - closest.length())>abs(averageLength -arr.get(j).length())){
                closest=arr.get(j);
            } else if(abs(averageLength - closest.length()) == abs(averageLength -arr.get(j).length())){
                    if(closest.length()<arr.get(j).length()){
                        closest=arr.get(j);
                    }
            }
        }
