...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        //spored .significance() da se najde najobrata carta
        SLLNode<Card> bestCard= firstSorcererCards.getFirst();
        SLLNode<Card> checkCard=bestCard.succ;
        while(checkCard != null){
            if(checkCard.element.significance()>bestCard.element.significance()){
                bestCard=checkCard;
            }
            checkCard=checkCard.succ;
        }
        //delete na cartata od firstSorcererCards
        firstSorcererCards.delete(bestCard);
        //insert na najodbrata karta vo sredina na secondSorcererCards
        SLLNode<Card> middleCard=secondSorcererCards.getFirst();
        for(int i=0;i<4;i++){
            if(middleCard.succ != null){
                middleCard = middleCard.succ;
            }else {
                break;
            }
        }
        secondSorcererCards.insertAfter(bestCard.element,middleCard);
    }
